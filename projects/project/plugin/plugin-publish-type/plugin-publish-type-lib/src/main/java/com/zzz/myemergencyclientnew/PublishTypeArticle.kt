...
import com.zzz.myemergencyclientnew.databinding.ActivityPublishTypeArticleBinding
...
interface PublishTypeArticleC {
    interface View : BaseView {
        fun openTypeDialog(resp: ArticleTypeResp)
    }

    abstract class Presenter :
        BasePresenter<View>() {
        abstract fun openTypeDialog(mActivity: AppCompatActivity, params: String, service: String)
        abstract fun netPublishTypeArticle(
...
class PublishTypeArticleP : PublishTypeArticleC.Presenter() {

    override fun netPublishTypeArticle(
        mActivity: AppCompatActivity,
        token: String,
        params: String,
        req: PublishTypeReq,
    ...
    override fun openTypeDialog(mActivity: AppCompatActivity, params: String, service: String) {
        OkGo.post<RespJson<List<ArticleTypeResp>>>(service + "/app/article/getArticleCate")
            .tag(this)
            .params("params", params)
            .execute(object : JsonCallback<RespJson<List<ArticleTypeResp>>>() {
                override fun onSuccess(response: Response<RespJson<List<ArticleTypeResp>>>) {
                    var dialog = MaterialDialog(mActivity)
                    val mAdapter = PublishArticleTypeAdapter()
                    mAdapter.onItemClickListener =
                        BaseQuickAdapter.OnItemClickListener { adapter, _, position ->
                            val entity = adapter.data[position] as ArticleTypeResp
...
class PublishTypeArticleActivity :
    BaseActivity<PublishTypeArticleP, ActivityPublishTypeArticleBinding>(),
    PublishTypeArticleC.View {
    private var images: ArrayList<ImageItem> = ArrayList<ImageItem>()
    private var imagePicker: ImagePicker = initImagePicker()
    private val req = PublishTypeReq()
    ...
    override fun getLayoutId(): Int {
        return R.layout.activity_publish_type_article
    }
...
//        ARouter.init(mContext.application)
        ...
        mPresenter.openTypeDialog(mContext, params, service)
    }
...
//                        req.imgs = pathStr;
//                        mPresenter.netPublishEmergKnowl(mActivity, req);
//                    }
//                });
                if (images != null) {
                    UploadUtils.uploadFileBatch(
                        ...
                        req.imgs = pathStr!!
                        mPresenter.netPublishTypeArticle(mContext, token, params, req, service)
                    }
...
//        GlobalKt.openVideoAddDialog(mActivity);
    ...
    override fun openTypeDialog(resp: ArticleTypeResp) {
    ...
    fun evnEvnImagePath(evn: EvnImagePath) {
        ...
        val adapter =
            ExpandImageAdapter(mContext, mViewBinding.gridview, images)
...
class PublishArticleTypeAdapter :
    BaseQuickAdapter<ArticleTypeResp, BaseViewHolder>(
        R.layout.item_publish_article_type,
        null
    ) {
    override fun convert(
        helper: BaseViewHolder,
        item: ArticleTypeResp
    ) {
        helper.setText(R.id.txt_title, item.name)
        val img_icon = helper.getView<ImageView>(R.id.img_icon)
        Glide.with(mContext).load(item.pathname).into(img_icon)
    }
}
