package com.zzz.myemergencyclientnew.dialogui.bean;
...
import com.zzz.myemergencyclientnew.dialogui.adapter.SuperAdapter;
import com.zzz.myemergencyclientnew.dialogui.config.DialogConfig;
import com.zzz.myemergencyclientnew.dialogui.listener.Buildable;
...
import com.zzz.myemergencyclientnew.dialogui.listener.Styleable;
...
public class BuildBean extends Buildable implements Styleable {
    ...
    public DialogUISoloListener soloListener;
    public DialogUIDateTimeSaveListener dateTimeListener;
    public DialogUIItemListener itemListener;

    /**
     * 魏文强，addNumber用于添加属性控制个数，strArray为传入的属性数组字符串
     */
    public int addNumber;
    public CharSequence strArray;
    public Map<String, String> map;
    public DialogUIAddListener addListener;
    ...
    @Override
    public Dialog showDialogHandler() {
        BuildBean bean = buildByType(this);
        if (dialog != null && !dialog.isShowing()) {
            ToolUtils.showDialogHandler(dialog, bean);
            return dialog;
        } else if (alertDialog != null && !alertDialog.isShowing()) {
            ToolUtils.showDialogHandler(alertDialog, bean);
            return alertDialog;
        }
        return null;
    }
