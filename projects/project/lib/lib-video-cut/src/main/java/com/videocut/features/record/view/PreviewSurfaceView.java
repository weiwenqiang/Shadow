package com.videocut.features.record.view;
...
public class PreviewSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private void startCamera(SurfaceHolder holder) throws IOException {
        ...
        try {
            ...
            /**
             * 此代码用于华为平板的兼容
             * 我们自己的原件所设置的宽高，在机器上并不适用于其特定的分辨率，这时我们应该获取机器本身所支持的Size，从中找到适合机器的那一个数值。
             */
            int PreviewWidth = 0;
            int PreviewHeight = 0;
            ...
            try {
                ...
                ex.printStackTrace();
                Toast.makeText(mContext, "尺寸计算失败", Toast.LENGTH_SHORT).show();
            }
        }
    }
