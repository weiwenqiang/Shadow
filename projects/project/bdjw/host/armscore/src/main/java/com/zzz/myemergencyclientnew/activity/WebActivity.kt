...
import com.zzz.myemergencyclientnew.databinding.ActivityWebBinding
...
class WebP : WebC.Presenter() {
}

@Route(path = C.Web)
class WebActivity :
    BaseActivity<WebP, ActivityWebBinding>(),
    WebC.View {
    ...
    override fun initView() {
        ...
        mAgentWeb.jsInterfaceHolder.addJavaObject("app_os", AndroidInterface(mAgentWeb, mContext))
...
class AndroidInterface(val agent: AgentWeb, private val activity: Activity) {
    @JavascriptInterface
    fun android_finish() {
        activity.finish()
    }
